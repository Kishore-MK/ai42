...
#### Flow:
1. **Estimation**: Calculate approximate cost based on prompt tokens
2. **Initial Payment**: Agent pays estimated amount via x402
3. **Streaming**: Tokens streamed in real-time to agent
4. **Settlement**: Final token count calculated
5. **Adjustment**: Refund overpayment or charge difference
...
#### Payment Flow:
1. Agent makes request to gateway endpoint
2. Gateway returns **402 Payment Required** with payment details
3. Agent's wallet signs and submits Solana transaction
4. Gateway verifies transaction on-chain
